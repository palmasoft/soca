...
class Vistas extends Base {
  ...
  public static function html($nombreVista, $varsVista = array(),
          $componente = '') {
    $response = '';
    if ($componente == '') {
  ...
  public static function cargar($nombreVista, $varsVista = array(),
          $componente = '') {
    if ($componente == '') {
      $componente = isset(self::$datos['componente']) ? strtolower(self::$datos['componente']) : $componente;
    }
  ...
  $nombreVista, $varsVista = array(), $componente = '', $desdeErrores = false) {
    if ($componente == '') {
      $componente = isset(self::$datos['componente']) ? strtolower(self::$datos['componente']) : $componente;
    }
    $ruta_interna = $componente . DS . DIR_VISTAS . DS . $nombreVista . EXT_VISTAS;
    $path_base = PATH_COMPONENTES . $ruta_interna;
    $path_base_css = PATH_COMPONENTES . $componente . DS . DIR_ESTILOS . DS . ( $desdeErrores ? 'errores' : self::$datos['controlador'] ) . ".css";
    $path_base_datos = PATH_COMPONENTES . $componente . DS . DIR_VISTAS . DS;
    ...
    if (!file_exists($path_base)) {
      if ($componente != '') {
        $ruta_interna = $componente . DS . $nombreVista . EXT_VISTAS;
        $path_base = Plantillas::$ruta . $ruta_interna;
      }
      if (!file_exists($path_base)) {
        $ruta_interna = 'sistema' . DS . DIR_VISTAS . DS . $nombreVista . EXT_VISTAS;
        $path_base = PATH_COMPONENTES . $ruta_interna;
        if (!file_exists($path_base)) {
          if ($desdeErrores) {
            echo " Error Cargando Vista " . $componente . "/" . $nombreVista;
...
//    }
    ...
    $msgSys = Config::get('mensajeOperacion');
    Config::set('mensajeOperacion', '');
    echo $msgSys;
    include $path_base;
